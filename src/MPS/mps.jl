...
function mps(
    ...
    res = Optim.optimize(a, b, show_trace = verbose) do λ
        sigma(u, λ, N; num_boundary, num_interior)
    end
...
function mps(
    ...
    res = Optim.optimize([λ₀], LBFGS()) do λ
        sigma(u, only(λ), N; num_boundary, num_interior)
    end
...
function mps!(
    ...
    λ = mps(u, a, b, N; num_boundary, num_interior, verbose)
...
function mps!(
    ...
    λ = mps(u, λ₀, N; num_boundary, num_interior, verbose)
